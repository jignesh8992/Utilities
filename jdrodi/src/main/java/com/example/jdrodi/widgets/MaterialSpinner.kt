...
open class MaterialSpinner @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, mode: Int = MODE_DROPDOWN
            ...
            adapter?.apply {
                if (value in 0 until count) {
                    ...
                    onItemSelectedListener?.onItemSelected(
                        this@MaterialSpinner, null, value, getItemId(value)
                    )
                } else {
                    editText.setText("")
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.MaterialSpinner).run {
            getInt(R.styleable.MaterialSpinner_android_gravity, -1).let {
                if (it > -1) {
                    ...
                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2) {
                        @SuppressLint("RtlHardcoded") if (isLayoutRtl()) {
            ...
            editText.isFocusableInTouchMode = getBoolean(
                R.styleable.MaterialSpinner_android_focusableInTouchMode, editText.isFocusableInTouchMode
            )
            ...
            getDimensionPixelSize(
                R.styleable.MaterialSpinner_android_textSize, -1
            ...
            getText(R.styleable.MaterialSpinner_android_text)?.let {
                ...
                if (isInEditMode) {
                    ...
                    throw RuntimeException(
                        "Don't set text directly." + "You probably want setSelection instead."
                    )
                }
            }
            ...
            colorStateList = context.obtainStyledAttributes(
                ...
                ColorStateList(
                    arrayOf(
                        intArrayOf(android.R.attr.state_pressed), intArrayOf(android.R.attr.state_focused), intArrayOf()
                    ), intArrayOf(activated, activated, normal)
                )
            }
            // Set the arrow and properly tint it.
            getContext().getDrawableCompat(
                getResourceId(R.styleable.MaterialSpinner_android_src, getResourceId(R.styleable.MaterialSpinner_srcCompat, R.drawable.ic_spinner_drawable)), getContext().theme
            ).let {
                setDrawable(it)
            }
    ...
    private fun Context.getDrawableCompat(
        @DrawableRes id: Int, theme: Resources.Theme?
    ): Drawable? {
    ...
    private inner class DialogPopup(
        val context: Context, private var prompt: CharSequence? = null
            ...
            setOnItemClickListener { parent, v, position, id ->
                ...
                onItemClickListener?.let {
                    this@MaterialSpinner.performItemClick(
                        v, position, adapter?.getItemId(position) ?: 0L
                    )
                }
                dismiss()
            }
    ...
    private inner class DropDownAdapter(
        private val adapter: SpinnerAdapter?, dropDownTheme: Resources.Theme?
    ...
    internal class SavedState : AbsSavedState {
        ...
        override fun toString(): String {
            return ("MaterialSpinner.SavedState{" + Integer.toHexString(System.identityHashCode(this)) + " selection=" + selection + ", isShowingPopup=" + isShowingPopup + "}")
        }
